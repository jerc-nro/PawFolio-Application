...
class _State extends ConsumerState<HomeScheduleCalendar> {
  ...
  Widget build(BuildContext context) {
    // Reset to today's month whenever Home tab is re-focused
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            for (int r = 0; r < rows; r++) ...[
              Row(
                ...
                children: List.generate(7, (c) {
                  final n = r * 7 + c - firstWD + 1;
                  if (n < 1 || n > daysInMonth) return const SizedBox(width: 34, height: 36);
                  final date    = DateTime(_month.year, _month.month, n);
                  ...
                  final overdue = records.any((r) =>
                      isPast && (r.status == 'Upcoming' || r.status == 'Ongoing'));
                  return _DayCell(day: n, isToday: isToday,
                      hasEvent: records.isNotEmpty && !overdue,
                      hasOverdue: overdue, records: records);
                }),
              ),
            ...
            // ── Legend ───────────────────────────────────────
            Row(children: const [
              _Dot(_orange, 'Today'),  SizedBox(width: 14),
              _Dot(_sage,   'Events'), SizedBox(width: 14),
              _Dot(_red,    'Overdue'),
            ]),
          ],
        ),
        ...
        children: [
          ...
          GridView.count(
            ...
            children: List.generate(12, (i) {
              final isSelected = i + 1 == _pickedMonth && _year == widget.selected.year;
              final isToday    = i + 1 == DateTime.now().month && _year == DateTime.now().year;
              return GestureDetector(
          ...
          // today shortcut
          GestureDetector(
            onTap: () => widget.onPick(DateTime(DateTime.now().year, DateTime.now().month)),
            child: Container(
...
class _DayCell extends StatelessWidget {
  ...
  const _DayCell({required this.day, required this.isToday,
      required this.hasEvent, required this.hasOverdue, required this.records});

  Color get _bg => isToday ? _orange
      : hasOverdue ? _red.withOpacity(0.25)
      : hasEvent   ? _sage.withOpacity(0.25)
      : Colors.transparent;

  @override
  Widget build(BuildContext context) => GestureDetector(
    onTap: records.isEmpty ? null : () => showModalBottomSheet(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: hasOverdue && !isToday
                ? Border.all(color: _red, width: 1.5) : null,
          ),
          child: Text('$day', style: TextStyle(
            fontSize: 12,
            color: isToday || records.isNotEmpty ? Colors.white : Colors.white54,
            fontWeight: isToday || records.isNotEmpty ? FontWeight.w700 : FontWeight.w400,
          )),
        ),
        if (hasEvent || hasOverdue)
          Container(width: 4, height: 4,
            ...
            decoration: BoxDecoration(
                color: hasOverdue ? _red : _sage, shape: BoxShape.circle)),
      ]),
    ),
  );
}
      ...
      child: Column(children: [
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            Text('${records.length} record${records.length > 1 ? "s" : ""} on this day',
...
class _Tile extends StatelessWidget {
  ...
  static const _icons = <String, IconData>{
    'Vaccination': Icons.verified_outlined, 'Medication': Icons.science_outlined,
    'Vet Visit': Icons.local_hospital_outlined, 'Grooming': Icons.content_cut_outlined,
  ...
  static const _sbg  = <String, Color>{
    'Done': Color(0xFFE8F5E9), 'Ongoing': Color(0xFFFFF8E1),
    'Upcoming': Color(0xFFE3F2FD), 'Overdue': Color(0xFFFFEBEE),
  };
  static const _scol = <String, Color>{
    'Done': Color(0xFF4CAF50), 'Ongoing': Color(0xFFFF9800),
    'Upcoming': Color(0xFF2196F3), 'Overdue': Color(0xFFE53935),
  };
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      ...
      Container(
        ...
        decoration: BoxDecoration(
            color: _sbg[r.status] ?? const Color(0xFFF5F5F5),
            borderRadius: BorderRadius.circular(20)),
        child: Text(r.status, style: TextStyle(fontSize: 11,
            fontWeight: FontWeight.w600,
            color: _scol[r.status] ?? const Color(0xFF9E9E9E))),
      ),
    ]),
  );
}
