...
enum _CardState { idle, loading, loaded, quotaExceeded, noInternet, error }
...
class _PetAiRecommendationCardState extends State<PetAiRecommendationCard>
  ...
  Future<void> _generate() async {
    ...
    final result = await _gemini.generatePetRecommendations(
    ...
    if (result.status == GeminiStatus.success) {
      setState(() {
        _rec = result.recommendation;
        _state = _CardState.loaded;
      });
      _fadeCtrl.forward();
    } else if (result.status == GeminiStatus.quotaExceeded) {
      setState(() => _state = _CardState.quotaExceeded);
    } else if (result.status == GeminiStatus.noInternet) {
      setState(() => _state = _CardState.noInternet);
    } else {
          ...
          child: Row(children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: kBrown.withValues(alpha: 0.25)),
              ),
  ...
  Widget _buildBody() {
    switch (_state) {
      ...
      case _CardState.quotaExceeded:
        return _InfoState(
          icon: Icons.hourglass_bottom_outlined,
          title: 'Daily limit reached',
          message:
              'The free tier allows 250 requests per day. Limit resets at midnight. Please try again later.',
          onRetry: _generate,
        );
      case _CardState.noInternet:
        return _InfoState(
          icon: Icons.wifi_off_outlined,
          title: 'No internet connection',
          message:
              'Firebase AI requires an internet connection. Please check your network and try again.',
          onRetry: _generate,
        );
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(
        ...
        style: const TextStyle(fontSize: 12, color: kLabel, height: 1.5),
      ),
      const SizedBox(height: 14),
      SizedBox(
        width: double.infinity,
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border: Border.all(color: kBrown.withValues(alpha: 0.2)),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Icon(icon, size: 15, color: kBrown.withValues(alpha: 0.8)),
        ...
        Text(message,
            style: const TextStyle(
                fontSize: 11, color: kLabel, height: 1.4)),
        const SizedBox(height: 12),
        GestureDetector(
          onTap: onRetry,
          child: Row(children: [
            Icon(Icons.refresh, size: 14, color: kNavy.withValues(alpha: 0.7)),
