...
final vaccineFilterProvider = StateProvider.autoDispose<String>((ref) => "ALL");

class VaccinationHistoryView extends ConsumerWidget {
  final Pet pet;
  const VaccinationHistoryView({super.key, required this.pet});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final currentFilter = ref.watch(vaccineFilterProvider);
              ...
              child: Padding(
                ...
                child: Column(children: [
                  ...
                  StatusFilterRow(
                    ...
                    onStatusSelected: (s) =>
                        ref.read(vaccineFilterProvider.notifier).state = s,
                  ),
                  const SizedBox(height: 10),
  ...
  void _handleEdit(BuildContext context, String uid, String docId,
    ...
    showEditRecordDialog(context,
        ...
        config: vaccinationEditConfig, existingData: data);
  }
  ...
  void _handleArchive(BuildContext context, WidgetRef ref, String docId) async {
    ...
    if (confirm) {
      await ref.read(recordControllerProvider.notifier).archiveRecord(
          petId: pet.petID, collection: 'vaccinations', recordId: docId);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Vaccination record archived")));
      }
    }
  }
    ...
    return Row(
      ...
      children: [
          ...
          onTap: uid == null
              ...
              : () => Navigator.push(context, MaterialPageRoute(
                    builder: (_) => ArchivedRecordsPage(
                        pet: pet, uid: uid, config: vaccinationArchiveConfig),
                  )),
        ...
        if (pet.isArchived)
          ...
          ElevatedButton.icon(
            onPressed: () =>
                showAddVaccinationDialog(context, pet.petID, pet.name, pet.type),
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return _vaccinationItem(context, ref,
              uid: uid, docId: docId, data: data,
              name: extra['vaccine_name'] ?? data['vaccine_name'] ?? 'Core Vaccine',
              type: extra['vaccine_type'] ?? data['vaccine_type'] ?? 'General',
              status: (data['status'] ?? 'UPCOMING').toString().toUpperCase(),
              date: data['date_string'] ?? 'N/A',
              time: extra['time'] ?? data['time_string'] ?? '--:--',
  ...
  Widget _vaccinationItem(BuildContext context, WidgetRef ref, {
    ...
    required String date, required String time, required String clinic, required String vet,
  }) {
    Color statusColor;
    switch (status) {
      case 'COMPLETED': statusColor = const Color(0xFF388E3C); break;
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          Expanded(child: _infoTile("ADMINISTERED", date, icon: Icons.calendar_today)),
          Expanded(child: _infoTile("TIME", time, icon: Icons.access_time)),
        ]),
  ...
  Widget _viewArchiveButton() => Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text("VIEW ARCHIVE", style: TextStyle(
  ...
  Widget _buildHeader(BuildContext context) => Padding(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text("VACCINATIONS", style: TextStyle(fontSize: 20,
  ...
  Widget _petProfileCard() => Container(
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(pet.breed, style: const TextStyle(color: Colors.white70, fontSize: 12)),
          ]),
        ]),
      );
