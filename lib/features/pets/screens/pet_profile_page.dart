...
class _PetProfilePageState extends ConsumerState<PetProfilePage> {
          ...
          sliver: SliverList(
           ...
           PetInfoCard(
            ...
            PetHistoryTile(
              ...
              onViewAll: () =>
                  _push(context, PreventativesHistoryView(pet: pet)),
            ),
            const SizedBox(height: 10),
            PetHistoryTile(
              title: 'Grooming',
  ...
  Future<void> _confirmSave(PetProfileNotifier notifier) async {
    ...
    if (_pendingFields.containsKey('breed')) {
      final breed = _pendingFields['breed'].toString();
      if (RegExp(r'\d').hasMatch(breed)) {
        validationError = 'Breed cannot contain numbers.';
      }
    }
    if (_pendingFields.containsKey('weight')) {
      final weight = _pendingFields['weight'].toString();
      if (double.tryParse(weight) == null) {
        validationError = 'Weight must be a valid number.';
      }
    }
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: (validationError != null ? kRed : kNavy).withOpacity(0.08),
                shape: BoxShape.circle,
              ),
              child: Icon(
                validationError != null ? Icons.warning_amber_rounded : Icons.save_outlined,
                color: validationError != null ? kRed : kNavy,
                size: 26,
              ),
            ),
            const SizedBox(height: 14),
            Text(validationError != null ? 'Invalid Input' : 'Save Changes?',
                style: TextStyle(
                    fontSize: 16,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: isAlive ? kGreen.withOpacity(0.1) : kRed.withOpacity(0.1),
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Container(
                ...
                height: 6,
                decoration: BoxDecoration(
                    color: isAlive ? kGreen : kRed, shape: BoxShape.circle),
              ),
...
class _SectionLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
        Container(
            ...
            height: 5,
            decoration:
                const BoxDecoration(color: kBrown, shape: BoxShape.circle)),
        const SizedBox(width: 8),
        Text(text,
            style: const TextStyle(
                fontSize: 11,
