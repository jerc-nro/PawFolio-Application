...
class MedicationHistoryView extends ConsumerWidget {
    ...
    if (confirm) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("Medication record archived"),
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasError) {
          debugPrint('Medications error: ${snapshot.error}');
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final freq   =
                "$count ${count == 1 ? 'time' : 'times'} a $period";
            final startDate =
                data['start_date'] ?? data['date_string'] ?? 'N/A';
            final endDateStr = data['end_date_string'] as String?;

            return _medicationItem(context, ref,
  ...
  Widget _medicationItem(BuildContext context, WidgetRef ref, {
    required String uid, required String docId,
    required Map<String, dynamic> data,
    required String name, required String type,
    required String status, required String startDate,
    String? endDate, required String frequency,
    required String dosage, required String clinic,
    required String vet, required String weight,
  }) {
    Color statusColor;
    switch (status) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start,
        ...
        Row(children: [
          Expanded(child: _infoTile("START DATE", startDate,
              icon: Icons.calendar_today)),
          Expanded(child: _infoTile("END DATE", endDate ?? '—',
              icon: Icons.event_available_outlined)),
        ]),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: _infoTile("FREQUENCY", frequency,
              icon: Icons.repeat)),
