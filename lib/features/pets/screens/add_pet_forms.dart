...
class StepBasicInfo extends StatefulWidget {
  ...
  final List<TextEditingController> otherBreedCtrls;
  final void Function(String) onTypeChanged;
  final void Function(List<String>) onBreedsChanged;
  final void Function(List<TextEditingController>) onOtherBreedCtrlsChanged;
  ...
  const StepBasicInfo({
    ...
    required this.otherBreedCtrls,
    required this.onTypeChanged,
    required this.onBreedsChanged,
    required this.onOtherBreedCtrlsChanged,
  });
...
class _StepBasicInfoState extends State<StepBasicInfo> {
  ...
  bool get _showOtherFields => widget.selectedBreeds.contains(_othersKey);

  // Total count = named chips + filled other text fields.
  // If Other is selected but all fields are empty → still counts as 1
  // so the counter shows immediately when the chip is tapped.
  int get _totalBreedCount {
    final namedCount = widget.selectedBreeds
        .where((b) => b != _othersKey)
        .length;
    if (!widget.selectedBreeds.contains(_othersKey)) return namedCount;
    final filledOther = widget.otherBreedCtrls
        .where((c) => c.text.trim().isNotEmpty)
        .length;
    return namedCount + (filledOther == 0 ? 1 : filledOther);
  }
  ...
  void _toggleBreed(String breed) {
    ...
    if (list.contains(breed)) {
      list.remove(breed);
      if (breed == _othersKey) {
        for (final c in widget.otherBreedCtrls) {
          c.dispose();
        }
        widget.onOtherBreedCtrlsChanged([TextEditingController()]);
      }
    } else {
      list.add(breed);
    }
  ...
  void _addOtherField() {
    final updated = List<TextEditingController>.from(widget.otherBreedCtrls)
      ..add(TextEditingController());
    widget.onOtherBreedCtrlsChanged(updated);
  }

  void _removeOtherField(int index) {
    final updated = List<TextEditingController>.from(widget.otherBreedCtrls);
  ...
  // Button logic per row:
  //   First & only → + only
  //   First, not last → + only (no – on first field ever)
  //   Middle → – only
  //   Last, not first → + on top, – below
  Widget _buildBreedButtons(int i,
    ...
    if (isFirst) {
      return _IconBtn(
        ...
        onTap: () => setState(_addOtherField),
      );
    }
    if (isLast) {
      // Both + and – side by side inside the 90px SizedBox
      return Row(
        ...
        children: [
          ...
          _IconBtn(
            ...
            onTap: () => setState(() => _removeOtherField(i)),
          ),
        ],
      );
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          children: [
            ...
            if (widget.selectedBreeds.isNotEmpty) ...[
              ...
              Row(children: [
                ...
                Text(
                  '$_totalBreedCount breed'
                  '${_totalBreedCount == 1 ? '' : 's'} selected',
                  style: const TextStyle(
                      fontSize: 11,
                      color: _kGreen,
                      fontWeight: FontWeight.w600)),
        ...
        // ── Other breed individual text fields ──────────────────────────────
        if (_showOtherFields) ...[
          const SizedBox(height: 14),
          _fieldCard(child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              _sectionLabel('SPECIFY OTHER BREED(S)'),
              const Text(
                'Each field is one breed. Tap + to add more.',
                style: TextStyle(fontSize: 11, color: _kLabel),
              ),
              const SizedBox(height: 10),
              ...List.generate(widget.otherBreedCtrls.length, (i) {
                final isFirst = i == 0;
                final isLast  = i == widget.otherBreedCtrls.length - 1;

                // Rows with both + and – need 42+6+42=90px for buttons.
                // First row only ever has +, so it stays full width.
                // Middle rows only have –, so also full width.
                // Only the last non-first row needs to shrink.
                final needsBothButtons = !isFirst && isLast;
                // button column width: single=42, double=42+6+42=90
                final btnWidth = needsBothButtons ? 90.0 : 42.0;
                ...
                return Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Expanded(
                        child: TextFormField(
                          controller: widget.otherBreedCtrls[i],
                          textCapitalization: TextCapitalization.words,
                          decoration: _input(
                            'Breed name ${i + 1}',
                          ...
                          validator: (v) {
                            if (!_showOtherFields) return null;
                      ...
                      SizedBox(
                        width: btnWidth,
                        child: _buildBreedButtons(
                            i, isFirst: isFirst, isLast: isLast),
                      ),
                    ],
                  ),
                );
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: InputDecoration(
                hintText: 'e.g. Loves cuddles, playful, afraid of loud noises…',
                hintStyle: const TextStyle(color: _kLabel, fontSize: 13),
                filled: true,
                fillColor: Colors.white,
                border: OutlineInputBorder(
...
class StepReview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = [
      ...
      _ReviewRow(Icons.notes_outlined,             'DESCRIPTION',
