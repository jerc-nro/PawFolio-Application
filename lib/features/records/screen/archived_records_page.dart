...
const groomingArchiveConfig = ArchivePageConfig(
  ...
  fields: [
    ...
    ArchiveFieldConfig(label: 'LOCATION', firestoreKey: 'clinic_name',  icon: Icons.location_on_outlined,  fallback: 'Not specified'),
  ],
);
  ...
  Future<void> _handleRestore(
    ...
    if (confirm) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Record restored to active list'),
            duration: Duration(seconds: 3),
          ),
        );
      }
          ...
          Row(children: [
            ...
            Expanded(
              child: _ActionButton(
                ...
                label: 'Edit',
                color: navBlue,
                muted: config.collection == 'weight_history',
                onTap: () => _handleEdit(context, data, docId),
              ),
            ),
            ...
            Expanded(
              child: _ActionButton(
                ...
                onTap: () => _handleRestore(context, ref, docId),
              ),
            ),
            const SizedBox(width: 8),
            ...
            Expanded(
              child: _ActionButton(
                ...
                onTap: () => _handleDelete(context, ref, docId),
              ),
            ),
          ]),
        ],
...
class _ActionButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final effectiveColor = muted ? Colors.grey.shade400 : color;
