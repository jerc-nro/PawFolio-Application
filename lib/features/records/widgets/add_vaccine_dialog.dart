...
class _VS extends ConsumerState<_VaccineContent> {
  ...
  Future<void> _save() async {
    final timeRequired =
        _status == 'COMPLETED' && _time.text.trim().isEmpty;
    setState(() {
      _nameErr = _name.text.trim().isEmpty
    ...
    try {
      ...
      if (mounted) {
        nav.pop();
        widget.onSaved?.call('Vaccination');
      }
    } catch (e) {
          ...
          children: [
            Row(children: [
              Container(
                ...
                decoration: BoxDecoration(
                    color: const Color(0xFFEDF4EB),
                    borderRadius: BorderRadius.circular(10)),
                child: const Icon(Icons.verified_outlined,
                    color: Color(0xFF5A9E62), size: 18),
              ),
            ...
            TextField(
                ...
                decoration: _deco(
                    hint: 'e.g. Rabies, DHPP', err: _nameErr != null)),
            ...
            _lbl(
                'Administration Time${_status == "COMPLETED" ? "" : " (optional)"}'),
            GestureDetector(
