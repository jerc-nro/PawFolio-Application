        ...
        .map((snapshot) {
          final pets = snapshot.docs
              .map((doc) =>
                  Pet.fromMap(doc.data() as Map<String, dynamic>, doc.id))
              .toList();
          // Archived (alive) pets appear first, deceased pets appear last
          pets.sort((a, b) {
            final aDeceased = !a.isAlive ? 1 : 0;
            final bDeceased = !b.isAlive ? 1 : 0;
            return aDeceased.compareTo(bDeceased);
          });
          return pets;
        });
  }
