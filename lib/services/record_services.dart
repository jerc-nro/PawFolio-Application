...
class RecordServices {
  ...
  return _collection(petId, collectionName)
    ...
    final records = s.docs.map((doc) {
      final record = PetRecord.fromDoc(doc, collectionName);

      if (record.status == 'Upcoming' && record.dateTimestamp != null) {
        final recordDay = DateTime(record.dateTimestamp!.year,
            record.dateTimestamp!.month, record.dateTimestamp!.day);
        if (recordDay.isBefore(todayDay)) {
          batch.update(doc.reference, {'status': 'Overdue'});
          hasBatchWrites = true;
          // Return with updated status immediately for UI
          return PetRecord(
            id: record.id,
            petID: record.petID,
            petName: record.petName,
